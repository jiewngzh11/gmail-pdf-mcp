...
但 **排程任務**（例如 `/schedule`）在無人值守環境執行，無法開啟瀏覽器，需改用 `STATIC_BEARER_TOKEN`。

靜態 token 驗證後，Server 會使用 **Azure Key Vault 中的 refresh token**（即部署者帳號，felix@kantech.com.tw）存取 Gmail 和 Drive。

在 `C:\Users\<使用者名稱>\.claude.json` 的 MCP 設定中加入 `headers`：
...
"mcpServers": {
  "gmail-pdf": {
    ...
    "url": "https://<Container App URL>/mcp",
    "headers": {
      "Authorization": "Bearer <STATIC_BEARER_TOKEN 的值>"
    }
  }
}
```

> **注意**：使用靜態 token 時，PDF 會存到**部署者的 Google Drive**（Key Vault 綁定的帳號），而非各自使用者的 Drive。互動式使用者仍用自己的瀏覽器授權，PDF 存到自己的 Drive，兩者互不干擾。
...
- **STATIC_BEARER_TOKEN 雙軌授權**：持有靜態 token 的請求（排程任務）走 Key Vault 的 Google 帳號；互動式使用者各自走瀏覽器授權。兩軌同時運作，互不影響。若需更換靜態 token，重新產生後至 Container App 更新環境變數即可。
...
A: 使用靜態 token 時，Server 固定使用 Key Vault 的 refresh token（部署者帳號）存取 Drive。這是預期行為，排程產生的 PDF 集中在部署者的 Drive，不會散落到各使用者 Drive。
